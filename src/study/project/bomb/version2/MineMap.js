...
function flagField(id){
	...
	var row = Number(id.split('/')[0]);
	var col = Number(id.split('/')[1]);
	data += '<img src="img/flag.png" onmousedown="holdCheck(id)" onmouseup="mouseCheck(id)" class ="flag" id =';
	data += id;
	data += '>';
	sweeper--;
	leftMine--;
...
function answer(){
	...
	for(var i=0; i<size; i++){
		...
		for(var j=0; j<size; j++){
			...
			}else if(answerField[i][j] == 9){
				ans += '<td><img src="img/mine2.png">';
			}else{
				ans += '<td><button type="button" style="background: rgb(225, 225, 225); color:red">';
				ans += answerField[i][j];
				ans += '</button>'
			}
			ans += '</td>';
